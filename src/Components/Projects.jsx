...
const Projects = () => {
  ...
  return (
    <div 
    className='container mx-auto py-4 pt-10 px-6 md:px-20 lg:px-32 my-5 w-full overflow-hidden' id='projects'>
      ...
      <div className='flex justify-end items-center mb-8'>
        <button onClick={prevProject} className='p-3 bg-gray-200 rounded mr-2 hover:bg-gray-300' aria-label='Previous Project'><img src={assets.left_arrow} alt="Prev" /></button>
        <button onClick={nextProject} className='p-3 bg-gray-200 rounded mr-2 hover:bg-gray-300' aria-label='Next Project'><img src={assets.right_arrow} alt="Next" /></button>
      </div>
        ...
        <div className='flex gap-8 transition-transform duration-500 ease-in-out'  style={{transform:`translateX(-${(currentIndex*100)/cardsToShow}%)`}}>
            {projectsData.map((project,index)=>(
                <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
                    ...
                    <div className='absolute left-0 right-0 bottom-5 felx justify-center'>
                        <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-sm'>
                        <h2 className='text-xl font-semibold text-gray-800'>{project.title}</h2>
                        <p className='text-gray-500 text-sm'>{project.price} <span> | </span>{project.location}</p></div>
                    </div>
                </div>
            ))}
        </div>
